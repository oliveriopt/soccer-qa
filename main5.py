...
def read_qa_query_plan(
    ...
    WHERE table_catalog = {database_l}
      ...
      AND (check_type IN UNNEST({check_types_lit})
        OR EXISTS (
            SELECT 1
            FROM UNNEST({check_types_lit}) t 
            WHERE STARTS_WITH(check_type, t)
        )
      )
    ORDER BY query_id
    """
...
class CompareObservedExpected(beam.DoFn):
    ...
    def process(self, observed_map: Dict, expected_map: Dict):
        ...
        alerts = []
        print(f"[INFO] QA | observed_map: {observed_map}")
        logging.info(f"observed_map '{observed_map}'")
        print(F"[INFO] QA | expected_map: {expected_map}")
        logging.info(f"expected_map '{expected_map}'")
