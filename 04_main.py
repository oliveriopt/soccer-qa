...
class CustomPipelineOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="List of QA check types separated by '|' (e.g., 'count_total_rows|nulls_per_column|duplicates_primary_key')."
        )
...
def generate_batch_queries(database, schema, table, batch_size, connection_string, reload_flag, type_of_extraction, primary_key, min_version_bq):
    ...
    try:
        ...
        logging.info(f"Max SYS_CHANGEVERSION: {max_ct_version}")
...
def read_qa_query_plan(
    ...
    # Literal interpolation to keep your style; ensure inputs are trusted.
...
# ---------- Parquet metrics computation & comparison (value_dist removed) ----------
...
def to_kv_from_qa(qa_row: dict) -> Iterable[Tuple[str, Any]]:
    ...
    NOTE: value_dist handling removed.
    """
    ...
    # Fallback: ignore other check types.
...
class CompareFn(beam.DoFn):
    ...
    Emits a dict with status:
      ...
      - MISSING_PARQUET
      - MISSING_BOTH
    NOTE: value_dist comparison removed.
    """
    def __init__(self, float_tol: float = 0.0):
...
def run_pipeline():
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        # Only numeric partials remain (counts and nulls and duplicates are ints)
        parquet_metrics = (
            parquet_partials
        ...
        alerts = (
            grouped
            | "Compare SQL vs Parquet" >> beam.ParDo(CompareFn(float_tol=0.0))
            # To output only mismatches, you could filter here:
