...
def read_qa_query_plan(
    ...
    Devuelve lista de dicts con: query_id, table_name, check_type, query_text, expected_output_format, table_catalog, table_schema
    """
    if not check_types:
    ...
    WHERE table_catalog = {database}
      ...
      AND check_type IN UNNEST({check_types})
    ORDER BY query_id
    """
    ...
    if result:
        ...
        logging.info(
            ...
            sample.get("check_type"), sample.get("expected_output_format")
        )
    else:
...
def run_pipeline():
    ...
    print(qa_check_types, type(qa_check_types))
